...
*/
    ...
    scanProgress: function (pdata,fid) {
        ...
        $('#PRG-'+fid).empty()
            ...
            $('#PRGT-'+fid).append('<span class="orange-text">'+pdata + '% </span>');
            $('.progress-bar').css('width', pdata+'%');
    },
   removeProgressBar: (fid)=>{
    $('#PRGT-'+fid).empty()
    $('#PRG-'+fid).empty()
    $('#PRGT-'+fid).append('<span class="display-8 purple-text small">Done </span> <i class="fa fa-check green-text"> <i> ');
...
function Body(data) {
    ...
    data.forEach(element => {
      ...
      //  console.log('WS Cost:'+ ws_cost)
        ...
        if (element.pType === 'LandLine') {
            table += `<td  style="text-align:right" scope="row" id=${element.pType}>${formatNumber(tuk_calls, { fractionDigits: 0 })}</td>`
        } else {
            if (element.pType == 'Mobile') {
                table += `<td style="text-align:right" scope="row" id=${element.pType}>${formatNumber(element.calls, { fractionDigits: 0 })}</td>`
            } else {
                table += `<td style="text-align:right" scope="row" id=${element.pType}>${formatNumber(element.calls, { fractionDigits: 0 })}</td>`
            }
        }
        table += `<td style="text-align:right" scope="row">${formatNumber(element.duration, { fractionDigits: 0 })}</td>`
        table += `<td style="text-align:right" scope="row">£${formatNumber(element.uCost, { fractionDigits: 2 })}</td>`
        table += `<td style="text-align:right" scope="row">£${formatNumber(element.wsCost, { fractionDigits: 2 })}</td></tr>`
    });
    ...
    table += `<td  style="text-align:right" scope="row">${formatNumber(tuk_calls, { fractionDigits: 0 })}</td>`
    table += `<td  style="text-align:right" scope="row">${formatNumber(tduration, { fractionDigits: 0 })}</td>`
    table += `<td  style="text-align:right" scope="row">£${formatNumber(u_cost, { fractionDigits: 2 })}</td>`
    table += `<td  style="text-align:right" scope="row">£${formatNumber(ws_cost, { fractionDigits: 2 })}</td></tr>`
    m_calls = 0
    count = 0;
    return table
}
...
function printableBody(data) {
    ...
    data.forEach(element => {
        ...
        table += `<td style="text-align:right" scope="row"  >${formatNumber(element.duration, { fractionDigits: 0 })}</td>`
        table += `<td style="text-align:right" scope="row"  >£${formatNumber(element.uCost, { fractionDigits: 2 })}</td></tr>`
    });
    ...
    table += `<td style="text-align:right" scope="row">${formatNumber(tuk_calls, { fractionDigits: 0 })}</td>`
    table += `<td style="text-align:right" scope="row">${formatNumber(tduration, { fractionDigits: 0 })}</td>`
    table += `<td style="text-align:right" scope="row">£${formatNumber(u_cost, { fractionDigits: 2 })}</td></tr>`
    m_calls = 0
    return table
}
function Footer() {
...
function fullBillBody(data) {
    ...
    data.forEach(element => {
      ...
      if(a[0] != pi){
        ...
        table += `<td style="text-align:right; color:#000">£${formatNumber((element.wholesaleCosts /1),{ fractionDigits: 2 })}</td></tr>`
    
    });
    table+='<tr><hr></tr>'
    table += ` <tr class="border_bottom"><td>Total</td><td></td><td></td><td></td><td style="text-align:right; color:#000; font-weight:bold">£${formatNumber((total_cost),{ fractionDigits: 2 })}</td></tr>`
