...
getFileName : function (path){
  ...
  runIndividual:(pType, dType)=>{
    ...
    gui.showTableCombine(dt,`${pType} Calls Duration [${ fn}]`, 'Total calls','Duration(minutes)','User Cost', 'WS Cost')
  },
  ukLandLines:()=>{
    gui.hideSummaryData()
    var uk= []
    ...
    gui.multiTable(uk, `UK Landlines(01*, 02*, 03*) [${ fn}]`,  'Total calls' ,'Duration(m)','User Cost', 'WS Cost')
  },
  ...
  SpecialNumbers:()=>{
    ...
    gui.multiTable(uk, `Special Nos(087, 084, 09) [${fn}]`,  'Total calls' ,'Duration(m)', 'User Cost', 'WS Cost')
  },
   ...
   runSummary:()=>{
    ...
    gui.multiTable(uk,`Bill Summary [${fn}]`,  'Total calls','Duration(m)','User Cost', 'WS Cost')
    gui.printableTable(uk,`Bill Summary [${fn}]`,  'Total calls','Duration(m)','User Cost', 'WS Cost')
    //printPdf(fn)
...
*/
     gui.FullBillTable(callData,`Bill Summary [${fn}]`,  'Destination',  'Destination Name','Duration(m)','Start Time', 'End Time', 'Cost')
...
*/
    //printPdf(fn)
     gui.printableFullBillTable(callData,`[${fn}]`,  'Date',  'Time','Destination','Usage', '£')
...
function findData(pType, dType) {
  ...
  if(pType == 'International'){
    dt.push({pType:pType,duration:minutes.Internationalcalls_durationInMinutes(phonesData,dType.duration),
    ...
    dt.push({pType:pType,duration:minutes.UKcalls_durationInMinutes(phonesData,dType.duration),
      calls:minutes.numberOfCalls(phonesData,dType.duration),
      uCost:minutes.calculateCost(phonesData,dType.costs),
      wsCost:minutes.calculateCost(phonesData,dType.wscosts)
    })
