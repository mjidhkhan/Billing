...
function checkToken () {
  /*if (!token) {
    return Promise.reject('variable not set\nSet it to a token with repo scope created from')
  } else {
...
function zipAssets () {
  ...
  const zipAssets = [{
    name: 'g3t-billing-app-mac.zip',
    path: path.join(outPath, 'G3T Billing-darwin-x64', 'G3T Billing.app')
  }, {
    name: 'g3t-billing-app-windows.zip',
    path: path.join(outPath, 'G3T Billing-win32-ia32')
  }, {
    name: 'g3t-billing-app-linux.zip',
    path: path.join(outPath, 'G3T Billing-linux-x64')
  }]
  ...
  return Promise.all(zipAssets.map(zipAsset)).then((zipAssets) => {
    return zipAssets.concat([{
      ...
      name: 'G3T BillingSetup.exe',
      path: path.join(outPath, 'windows-installer', 'G3T Billing.exe')
    }, {
      name: `G3T-Billing-${version}-full.nupkg`,
      path: path.join(outPath, 'windows-installer', `G3T-Billing-${version}-full.nupkg`)
    }])
  })
}
